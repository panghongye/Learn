...
impl Category {
    pub async fn new(pool: &MySqlPool, title: &str) -> Result<Self> {
        sqlx::query_as!(
            Self,
    ...
    pub async fn get(pool: &MySqlPool, id: i64) -> Result<Option<Self>> {
        sqlx::query_as!(Self, r#"select * from category where id = ?"#, id)
            .fetch_optional(pool)
            .await
    }

    pub async fn tasks(&self, pool: &MySqlPool) -> Result<Vec<Task>> {
...
impl Task {
    pub async fn new(pool: &MySqlPool, title: &str, category: &Category) -> Result<Self> {
        sqlx::query_as!(
            Self,
...
impl Entry {
    pub async fn new_now(pool: &MySqlPool, task: &Task) -> Result<Self> {
        let now = Utc::now();
        sqlx::query_as!(
            Self,
