...
#[get("/categories")]
pub async fn list_categories(db: &State<MySqlPool>) -> Result<Json<Vec<Category>>> {
    let categories = Category::all(&**db).await?;
    Ok(Json(categories))
}

#[get("/categories/<id>")]
pub async fn get_category(db: &State<MySqlPool>, id: i64) -> Result<Option<Json<Category>>> {
